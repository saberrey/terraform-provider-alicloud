...
* `runtime` - (Deprecated from version 1.174.0)(Optional, Available in 1.103.2+) The runtime of containers. Default to `docker`. If you select another container runtime, see [Comparison of Docker, containerd, and Sandboxed-Container](https://www.alibabacloud.com/help/doc-detail/160313.htm). Detailed below.
* `enable_ssh` - (Deprecated from version 1.174.0)(Optional) Enable login to the node through SSH. Default to `false`.
* `rds_instances` - (Deprecated from version 1.174.0)(Optional, Available in 1.103.2+) RDS instance list, You can choose which RDS instances whitelist to add instances to.
...
* `exclude_autoscaler_nodes` - (Deprecated from version 1.174.0)(Optional, Available in 1.88.0+) Exclude autoscaler nodes from `worker_nodes`. Default to `false`.
...
* `pod_vswitch_ids` - (Optional) - [Terway Specific] The vswitches for the pod network when using Terway.Be careful the `pod_vswitch_ids` can not equal to `worker_vswitch_ids` or `master_vswtich_ids` but must be in same availability zones.
...
* `worker_number` - (Deprecated from version 1.174.0)(Optional) The worker node number of the kubernetes cluster. Default to 3. It is limited up to 50 and if you want to enlarge it, please apply white list or contact with us. From version 1.109.1, It is not necessary in the professional managed cluster, but it is necessary in other types of clusters.
* `worker_instance_types` - (Deprecated from version 1.174.0)(Optional) The instance type of worker node. Specify one type for single AZ Cluster, three types for MultiAZ Cluster. From version 1.109.1, It is not necessary in the professional managed cluster, but it is necessary in other types of clusters.
* `password` - (Deprecated from version 1.174.0)(Optional, Sensitive) The password of ssh login cluster node. You have to specify one of `password` `key_name` `kms_encrypted_password` fields. From ersion 1.109.1, It is not necessary in the professional managed cluster.
* `key_name` - (Deprecated from version 1.174.0)(Optional) The keypair of ssh login cluster node, you have to create it first. You have to specify one of `password` `key_name` `kms_encrypted_password` fields. From ersion 1.109.1, It is not necessary in the professional managed cluster.
* `kms_encrypted_password` - (Deprecated from version 1.174.0)(Optional, Available in 1.57.1+) An KMS encrypts password used to a cs kubernetes. You have to specify one of `password` `key_name` `kms_encrypted_password` fields.
* `kms_encryption_context` - (Deprecated from version 1.174.0)(Optional, MapString, Available in 1.57.1+) An KMS encryption context used to decrypt `kms_encrypted_password` before creating or updating a cs kubernetes with `kms_encrypted_password`. See [Encryption Context](https://www.alibabacloud.com/help/doc-detail/42975.htm). It is valid when `kms_encrypted_password` is set.
* `worker_instance_charge_type` - (Deprecated from version 1.174.0)(Optional) Worker payment type, its valid value is either or `PostPaid` or `PrePaid`. Defaults to `PostPaid`. If value is `PrePaid`, the files `worker_period`, `worker_period_unit`, `worker_auto_renew` and `worker_auto_renew_period` are required.
* `worker_period` - (Deprecated from version 1.174.0)(Optional) Worker payment period. The unit is `Month`. Its valid value is one of {1, 2, 3, 6, 12, 24, 36, 48, 60}.
* `worker_period_unit` - (Deprecated from version 1.174.0)(Optional) Worker payment period unit, the valid value is `Month`.
* `worker_auto_renew` - (Deprecated from version 1.174.0)(Optional) Enable worker payment auto-renew, defaults to false.
* `worker_auto_renew_period` - (Deprecated from version 1.174.0)(Optional) Worker payment auto-renew period,, it can be one of {1, 2, 3, 6, 12}.
* `worker_disk_category` - (Deprecated from version 1.174.0)(Optional) The system disk category of worker node. Its valid value are `cloud`, `cloud_ssd`, `cloud_essd` and `cloud_efficiency`. Default to `cloud_efficiency`.
* `worker_disk_size` - (Deprecated from version 1.174.0)(Optional) The system disk size of worker node. Its valid value range [40~500] in GB. Default to 40.
* `worker_data_disks` - (Deprecated from version 1.174.0)(Optional, Available in 1.91.0+) The data disk configurations of worker nodes, such as the disk type and disk size.
  * `category` - (Deprecated from version 1.174.0)The type of the data disks. Valid values: `cloud`, `cloud_efficiency`, `cloud_ssd` and `cloud_essd`. Default to `cloud_efficiency`.
  * `size` - (Deprecated from version 1.174.0)The size of a data disk, at least 40. Unit: GiB.
  * `encrypted` - (Deprecated from version 1.174.0)Specifies whether to encrypt data disks. Valid values: true and false. Default to `false`.
  * `performance_level` - (Deprecated from version 1.174.0)(Optional, Available in 1.120.0+) Worker node data disk performance level, when `category` values `cloud_essd`, the optional values are `PL0`, `PL1`, `PL2` or `PL3`, but the specific performance level is related to the disk capacity. For more information, see [Enhanced SSDs](https://www.alibabacloud.com/help/doc-detail/122389.htm). Default is `PL1`.
  * `auto_snapshot_policy_id` - (Deprecated from version 1.174.0)(Optional, Available in 1.120.0+) Worker node data disk auto snapshot policy.
* `node_name_mode` - (Deprecated from version 1.174.0)(Optional, Available in 1.88.0+) Each node name consists of a prefix, an IP substring, and a suffix. For example, if the node IP address is 192.168.0.55, the prefix is aliyun.com, IP substring length is 5, and the suffix is test, the node name will be `aliyun.com00055test`.
* `node_port_range`- (Deprecated from version 1.174.0)(Optional, ForceNew, Available in 1.103.2+) The service port range of nodes, valid values: `30000` to `65535`. Default to `30000-32767`.
* `os_type` - (Deprecated from version 1.174.0)(Optional, ForceNew, Available in 1.103.2+) The operating system of the nodes that run pods, its valid value is either `Linux` or `Windows`. Default to `Linux`.
* `platform` - (Deprecated from version 1.174.0)(Optional, ForceNew, Available in 1.103.2+) The architecture of the nodes that run pods, its valid value is either `CentOS` or `AliyunLinux`. Default to `CentOS`.
* `image_id` - (Deprecated from version 1.174.0)(Optional) Custom Image support. Must based on CentOS7 or AliyunLinux2.
* `cpu_policy` - (Deprecated from version 1.174.0)(Optional) Kubelet cpu policy. For Kubernetes 1.12.6 and later, its valid value is either `static` or `none`. Default to `none`.
* `user_data` - (Deprecated from version 1.174.0)(Optional, Available in 1.81.0+) Custom data that can execute on nodes. For more information, see [Prepare user data](https://www.alibabacloud.com/help/doc-detail/49121.htm).
* `taints` - (Deprecated from version 1.174.0)(Optional, Available in 1.103.2+) Taints ensure pods are not scheduled onto inappropriate nodes. One or more taints are applied to a node; this marks that the node should not accept any pods that do not tolerate the taints. For more information, see [Taints and Tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/). Detailed below.
* `worker_disk_performance_level` - (Deprecated from version 1.174.0)(Optional, Available in 1.120.0+) Worker node system disk performance level, when `worker_disk_category` values `cloud_essd`, the optional values are `PL0`, `PL1`, `PL2` or `PL3`, but the specific performance level is related to the disk capacity. For more information, see [Enhanced SSDs](https://www.alibabacloud.com/help/doc-detail/122389.htm). Default is `PL1`.
* `worker_disk_snapshot_policy_id` - (Deprecated from version 1.174.0)(Optional, Available in 1.120.0+) Worker node system disk auto snapshot policy.
* `install_cloud_monitor` - (Deprecated from version 1.174.0)(Optional) Install cloud monitor agent on ECS. Default to `true`.
...
* `worker_nodes` - (Deprecated from version 1.174.0) List of cluster worker nodes.
  * `id` - (Deprecated from version 1.174.0) ID of the node.
  * `name` - (Deprecated from version 1.174.0) Node name.
  * `private_ip` - (Deprecated from version 1.174.0) The private IP address of node.
